...
if __name__=='__main__':
    ...
    for i in grid_type: #change to select specific grid
        ...
        for data in dataset1:     #change dataset
            ...
            for patch in patches:
                R,G,B=Per_Channel_Color_Histogram(patch,interval)
                R=l1_norm(R)
                G=l1_norm(G)
                B=l1_norm(B)
                channels = [R,G,B]
        ...
        for Qhist1 in hist_of_query:
            ...
            for Shist1 in hist_of_support:
                for k in range(len(hist_of_query_patches)):
                    result_r = KL_divergence(Qhist1[k][0],Shist1[k][0])
                    result_g = KL_divergence(Qhist1[k][1],Shist1[k][1])
                    result_b = KL_divergence(Qhist1[k][2],Shist1[k][2])
                    temp_result = (result_r+result_b+result_g)/3
